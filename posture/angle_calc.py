...
def determine_faulty_joints_from_rula_reba(score):
    """
    Determines faulty joints based on the RULA/REBA score.

    Parameters:
    - score (int): The final ergonomic risk assessment score.

    Returns:
    - faulty_joints (list): List of joints contributing to poor posture.
    """

    # Define thresholds for different risk levels
    risk_mapping = {
        (1, 2): [],  # No significant risk
        (3, 4): ["wrist", "elbow"],  # Mild risk, usually in upper extremities
        (5, 6): ["shoulder", "spine"],  # Moderate risk, affecting major posture areas
        (7, 8): ["neck", "lower back"],  # High risk, critical posture problems
        (9, 10): ["neck", "shoulder", "spine", "wrist"],  # Severe risk, multiple joints affected
        (11, 12): ["whole body"],  # Very severe, urgent action required
    }

    faulty_joints = []
    for (lower, upper), joints in risk_mapping.items():
        if lower <= score <= upper:
            faulty_joints = joints
            break
