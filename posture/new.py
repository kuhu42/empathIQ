...
def determine_faulty_joints(joint_angles):
    ...
    Generate dynamic prompts for each misaligned joint.
    """
    acceptable_ranges = {
        "neck": (160, 180),
        "left_shoulder": (30, 60),
    ...
    detailed_prompts = {
        "neck": lambda angle: "Lift your neck" if angle < 160 else "Lower your neck",
        "left_shoulder": lambda angle: "Lift your left shoulder" if angle < 30 else "Lower your left shoulder",
        "right_shoulder": lambda angle: "Lift your right shoulder" if angle < 30 else "Lower your right shoulder",
        "spine": lambda angle: "Straighten your back" if angle < 150 else "Relax your back",
        "left_elbow": lambda angle: "Straighten your left arm" if angle < 30 else "Bend your left elbow",
        "right_elbow": lambda angle: "Straighten your right arm" if angle < 30 else "Bend your right elbow"
    }
    ...
    for joint, angle in joint_angles.items():
        if joint in acceptable_ranges:
            ...
            if angle < lower_bound or angle > upper_bound:
                # Generate dynamic prompt based on the joint angle misalignment
                faulty_joints[joint] = detailed_prompts[joint](angle)
